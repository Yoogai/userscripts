...
(() => {
  ...
  console.log('[Почта Адыгеи Redesign v3.1.19] Скрипт инициализирован');
  ...
  function installRecipientMenu() {
    ...
    const rememberRecent = () => {
      const found = [...document.querySelectorAll('.ui-autocomplete li')].map((item) => {
        const address = item.textContent.match(/[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}/)?.[0];
        return address ? { name: item.textContent.replace(address, '').replace(/[<>\"]/g, '').trim(), address } : null;
      }).filter(Boolean);
      if (!found.length) return;
      const merged = [...found, ...recentRecipients].filter((entry, index, list) => list.findIndex((item) => item.address.toLowerCase() === entry.address.toLowerCase()) === index).slice(0, 20);
      recentRecipients = merged;
      GM_setValue(KEYS.recentRecipients, recentRecipients);
    };
    const stockObserver = new MutationObserver(() => {
      const before = recentRecipients.length;
      rememberRecent();
      if (recentRecipients.length !== before && menu.classList.contains('is-open')) render();
    });
    ...
    const render = () => {
      ...
      const recent = recentRecipients.filter((entry) => entry?.address && !added.has(entry.address.toLowerCase()) && (!query || `${entry.name || ''} ${entry.address}`.toLocaleLowerCase('ru').includes(query)));
      const button = ([name, address]) => `<button type="button" class="ady-recipient-item" role="option" data-address="${address}"><span class="ady-recipient-name">${name || address}</span><span class="ady-recipient-address">${address}</span></button>`;
      menu.innerHTML = `${primary.length ? '<div class="ady-recipient-section">Закреплённые</div>' : ''}${primary.map(button).join('')}${recent.map(({name, address}) => button([(name || '').replace(/[<>\"]/g, '').trim(), address])).join('')}`;
    ...
    const open = (input) => { activeInput = input; place(); if (render()) menu.classList.add('is-open'); };
    ...
    menu.addEventListener('mousedown', (event) => {
      ...
      window.setTimeout(() => { input.focus(); place(); render(); menu.classList.add('is-open'); }, 80);
    });
    document.addEventListener('click', (event) => {
      const input = getInput();
